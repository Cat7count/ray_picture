...
import com.ray.raypicturebackend.api.aliyunai.model.GetOutPaintingTaskResponse;
import com.ray.raypicturebackend.api.imagesearch.ImageSearchApiFacade;
...
public class PictureController {
    ...
    private final Cache<String, String> LOCAL_CACHE =
            Caffeine.newBuilder().initialCapacity(1024) // 初始化缓存
                    .maximumSize(10000L) // 最大条数
                    // 缓存 5 分钟移除
    ...
    /**
     * 分页获取图片列表（封装类） 带分布式缓存技术 Caffeine + Redis
     */
    ...
    public BaseResponse<Page<PictureVo>> listPictureVOByPageWithCache(@RequestBody PictureQueryRequest pictureQueryRequest,
        ...
        String hashKey = DigestUtils.md5DigestAsHex(queryCondition.getBytes()); // 通过MD5减少key的长度
        String cacheKey = "raypicture:listPictureVoByPage:" + hashKey;
        // 先查本地缓存
        ...
        if (cacheValue != null) {
            LOCAL_CACHE.put(cacheKey, cacheValue); // Redis命中更新一下本地缓存
            Page<PictureVo> cachePage = JSONUtil.toBean(cacheValue, Page.class);
            return ResultUtils.success(cachePage);
        }
        // 若都没有就要查询数据库然后更新本地缓存和分布式缓存
        Page<Picture> picturePage = pictureService.page(new Page<>(current, size),
                pictureService.getQueryWrapper(pictureQueryRequest));
        String updateCacheValue = JSONUtil.toJsonStr(picturePage);
        LOCAL_CACHE.put(cacheKey, updateCacheValue);
        opsForValue.set(cacheKey, updateCacheValue,5,TimeUnit.MINUTES); // 一定要设置过期时间
        // 获取封装类
        return ResultUtils.success(pictureService.getPictureVOPage(picturePage, request));
    }
    ...
    public BaseResponse<List<ImageSearchResult>> searchPictureByPicture(@RequestBody SearchPictureByPictureRequest searchPictureByPictureRequest) {
        ...
        ThrowUtils.throwIf(oldPicture == null, ErrorCode.NOT_FOUND_ERROR);
        List<ImageSearchResult> resultList = ImageSearchApiFacade.searchImage(oldPicture.getOriginalUrl()); // 因为webp格式不支持以图搜图
